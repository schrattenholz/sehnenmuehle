    ...
    offcanvasSidebar: () => {
      ...
      for (let i = 0; i < openTogglers.length; i++) {
        ...
        openTogglers[i].addEventListener('click', (e) => {
          e.preventDefault();
          let sidebarID = e.currentTarget.getAttribute('data-target');
    ...
    ajaxifySubscribeForm: () => {
      ...
      function register($form) {
        ...
        $.ajax({
          ...
          error: function (err) { alert('Konnte nicht verbinde. Bitte versuchen Sie es später erneut. ') },
          success: function (data) {
            if(data.result === 'success') {
              $status.removeClass('status-error').addClass('status-success').text('Danke für die Teilnahme an unserem Newsletter. Wir haben Ihnen eine E-Mail gesendet, mit der Sie Ihre Teilnahme bestätigen müssen.')
